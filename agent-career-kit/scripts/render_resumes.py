...
def render_claim_sections(profile: dict[str, Any], view: dict[str, Any]) -> str:
    ...
    sections: list[str] = []
    for title, categories in SECTION_ORDER:
...
def render_education(education: list[dict[str, Any]]) -> str:
    public = [item for item in education if item.get("status") in {"provided", "confirmed"}]
    if not public:
        return ""
...
def render_resumes(workspace: Path) -> list[Path]:
    profile = require_valid_profile(workspace)
    rendered = {view_name: render_document(profile, view_name) for view_name in ("development", "algorithm")}
    outputs: list[Path] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render the two stable Agent resume views to LaTeX.")
    parser.add_argument("workspace")
    args = parser.parse_args()
    outputs = render_resumes(workspace_path(args.workspace))
