...
你不需要一次性准备完整材料。最小输入可以只有一份简历、一段项目说明、一个 GitHub 链接或一个目标 JD；材料越完整，输出越稳定。

| 你提供什么 | Skill 会做什么 | 主要输出 |
|---|---|---|
| 现有简历 PDF / DOCX / TeX / Markdown | 抽取经历、技能、项目、数字和缺口，做简历审计 | `candidate-profile.json`、`intake.md`、`outputs/resumes/resume-audit.md` |
| GitHub、项目文档、论文、博客、Demo 链接 | 建立项目证据链，判断能写到简历和作品集的内容 | 项目证据清单、能力画像、README / Demo / 作品集建议 |
| Agent 开发或 Agent 算法目标 | 按方向筛选重点，形成开发版或算法版主简历 | `outputs/resumes/development/`、`outputs/resumes/algorithm/` |
| 大厂 JD 或目标公司信息 | 拆解岗位要求、匹配证据、识别面试风险 | `outputs/applications/<id>/`、公司准备包、面试题池 |
| 面试记录或弱项反馈 | 做复盘，更新弱点和下一轮训练任务 | `weaknesses.md`、Story Bank、复训计划 |
| 投递、面试、Offer 状态 | 维护求职漏斗和下一步优先级 | `application-dashboard.md`、`outputs/career-dashboard/` |

## 典型使用 Case

### Case 1：我只有一份旧简历

输入：上传简历文件，并说明目标是 Agent 开发还是 Agent 算法。

输出：候选人档案、30 秒筛选印象、逐段简历问题、待确认事实、下一步补材料清单。

```text
使用 agent-career-kit。我只有这份旧简历，目标是 Agent 开发岗位。
请先建立候选人档案并审计简历，不要改格式，不要编造缺失信息。
```

### Case 2：我想生成两份稳定主简历

输入：简历、项目资料、GitHub 或论文材料。

输出：Agent 开发版和 Agent 算法版 LaTeX / PDF / Overleaf 包；两份简历使用同一事实档案，但突出重点不同。

```text
基于我的候选人档案生成 Agent 开发版和 Agent 算法版简历。
开发版突出工程交付、工具调用、工作流、评估和可靠性；
算法版突出模型、训练、检索、实验设计和指标。
```

### Case 3：我拿到一个大厂 JD

输入：JD 文本和已有候选人 workspace。

输出：JD 要求拆解、证据映射、风险点、面试准备包和独立投递快照；不会改写长期主档案。

```text
这是字节 Agent 开发岗 JD。请生成要求-证据-缺口映射、
公司准备包和一份独立投递快照。任何新表达先让我确认。
```
...
输出：单点 grill-me 深挖，或 45-60 分钟完整模拟面试；结束后输出评分、弱点、参考答法结构和复训任务。

```text
基于我的简历做一场 60 分钟 Agent 算法模拟面试。
流程包含自我介绍、项目深挖、Agent 八股、Coding、系统设计和反问。
一次只问一题，结束后复盘。
```

### Case 5：我要做公开作品集

输入：项目资料、GitHub、Demo、论文或博客链接，以及允许公开的联系方式。

输出：GitHub README、项目讲解稿、个人求职页、简历下载入口和公开事实检查清单。

```text
把我的项目整理成 Story Bank、GitHub README 和个人求职页。
先列出哪些事实可以公开，哪些还需要我确认。
```
...
```mermaid
flowchart LR
    A["任意候选人材料<br/>简历 / GitHub / 项目 / 论文 / JD / 面试记录"] --> B["材料归档与事实抽取"]
    B --> C["长期候选人档案<br/>阶段画像 + Agent 开发 / 算法能力画像"]
    C --> D["按需主简历<br/>开发版 / 算法版 LaTeX、PDF、Overleaf"]
    C --> E["项目与学习补强<br/>项目证据、实验、README、Demo"]
    C --> F["面试与题库训练<br/>单点深挖 / 45-60 分钟模拟面试"]
    D --> G["作品集与 JD 投递快照<br/>个人求职页 / 公司准备包"]
    E --> G
    F --> H["面后复盘<br/>弱点、Story Bank、复训任务"]
    G --> I["投递 → 面试 → Offer<br/>漏斗、下一行动、Offer 对比"]
    I --> H
    H --> C
```
...
- 目标就是帮助候选人系统冲刺年薪百万级 Offer：提升材料命中率、项目说服力、面试稳定性和复盘效率。最终结果仍取决于候选人真实能力、岗位机会、市场环境和面试表现。
- 可以基于候选人已有事实做简历润色、项目包装建议、能力补强路线、Demo 方案、面试话术和投递策略；也可以给出更强版本的候选表达草案，供用户确认或补证。
- 对外发布的简历、作品集和投递快照只使用候选人已提供或已确认的事实；待补证的计划、建议和升级方向会标成补强项，不直接写成已完成成果。
- 不虚构学校、公司、角色、指标、论文、奖项或项目结果。
